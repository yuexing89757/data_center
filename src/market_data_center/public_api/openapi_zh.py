"""Chinese annotations for the public FastAPI OpenAPI contract."""

# ruff: noqa: RUF001 - Chinese descriptions intentionally use Chinese punctuation.

from typing import Any

FIELD_DESCRIPTIONS_ZH = {
    "cursor": "上一页返回的不透明游标，绑定接口、交易日、计算版本及每页上限。",
    "calculation_status": "已发布计算批次状态；PARTIAL 表示存在输入不完整证券。",
    "completed_at": "计算批次完成时间，按上海时区格式化。",
    "event_watermark": "计算采用的正式事件观察水位，晚到事件不混入旧版本。",
    "rule_set_version": "交易所规则集版本。",
    "coverage": "所选计算版本的证券覆盖统计。",
    "expected_count": "计算候选证券总数。",
    "complete_count": "输入完整的证券数。",
    "incomplete_count": "输入不完整的证券数。",
    "not_applicable_count": "不适用当前规则的证券数。",
    "next_cursor": "下一页游标；没有更多记录时为空。",
    "segment": "监管规则所属板块：沪市主板、深市主板或创业板。",
    "calculated_state": "系统计算达到条件的状态，不等同于交易所正式认定。",
    "announced_state": "事件水位以内交易所已经正式公告的状态。",
    "triggered_rules": "该股票达到条件且输入完整的规则列表。",
    "rule_code": "版本化监管规则编码。",
    "kind": "规则公式类型：累计偏离、换手率复合条件或正式事件次数。",
    "window_start_date": "规则实际计算窗口的起始交易日。",
    "window_end_date": "规则实际计算窗口的结束交易日。",
    "observed_window_days": "实际参与计算的交易日数。",
    "current_value": "当前规则观测值，含义取决于规则类型。",
    "threshold": "当前规则的触发阈值。",
    "secondary_current_value": "复合规则的第二观测值；不适用时为空。",
    "secondary_threshold": "复合规则的第二阈值；不适用时为空。",
    "event_count": "规则窗口内符合条件的正式事件次数。",
    "required_count": "次数型规则要求的事件次数。",
    "selected_reset_date": "本次规则计算采用的官方重置日期。",
    "next_trade_date": "查询日期之后的下一交易日。",
    "lookback_trading_days": "正式异动回看交易日数，固定为30并包含查询日。",
    "lookback_start_date": "最近30个交易日中最早的交易日。",
    "official_event_count_30d": "最近30个交易日内、水位以内的正式异动事件数。",
    "latest_source_event_id": "最近一次交易所正式事件的来源标识。",
    "latest_event_date": "最近一次正式事件的统计期间结束日。",
    "latest_event_published_at": "最近一次正式事件公告时间，按上海时区格式化。",
    "latest_event_level": "最近一次正式事件的异常级别。",
    "latest_event_direction": "官方披露的异动方向；未明确披露时为空。",
    "latest_event_source_title": "最近一次正式事件的官方公告标题。",
    "latest_event_source_url": "最近一次正式事件的官方来源链接。",
    "next_triggers": "下一交易日各规则和指数情景的条件测算，不是价格预测。",
    "applicability": "该股票是否适用监管异动规则：适用、不适用或输入不足。",
    "data_completeness": "该股票规则测算的输入完整性。",
    "is_triggered_today": "查询日收盘是否达到异常或严重异常条件；不等同于交易所正式认定。",
    "stock_daily_return_pct": "查询日个股涨跌幅，单位为百分比。",
    "benchmark_close": "查询日基准指数收盘点位。",
    "benchmark_daily_return_pct": "查询日基准指数涨跌幅，单位为百分比。",
    "daily_deviation_pct": "查询日个股与基准指数涨跌幅之差，单位为百分比。",
    "official_event_direction_30d": "最近30个交易日内最近一次正式异动的方向；无事件时为空。",
    "abnormal_count_10d": "最近10个交易日内系统测算的异动次数，含上涨与下跌。",
    "abnormal_count_10d_up": "最近10个交易日内系统测算的上涨方向异动次数。",
    "abnormal_count_10d_down": "最近10个交易日内系统测算的下跌方向异动次数。",
    "benchmark_symbol": "规则采用的基准指数标准代码。",
    "scenario_code": "指数下跌2%、持平、上涨2%，或已触发、不可按价格反解的情景。",
    "scenario_index_pct": "下一交易日基准指数情景涨跌幅，单位为百分比。",
    "next_day_reference_price": "下一交易日涨跌幅计算所用的官方参考价。",
    "raw_trigger_price": "按规则反解、尚未按0.01元取整的触发价。",
    "trigger_price": "按方向取整并复核后的下一交易日所需收盘价。",
    "trigger_change_pct": "触发收盘价相对参考价的涨跌幅，单位为百分比。",
    "lower_limit_price": "下一交易日适用的跌停价。",
    "upper_limit_price": "下一交易日适用的涨停价。",
    "reachability": "下一交易日可达、不可达、已触发或不能仅按价格计算。",
    "requires_official_event_confirmation": "是否仍需交易所正式确认新事件后才能计入次数。",
    "accepted_auction_row_count": "通过校验的竞价记录数。",
    "algorithm_version": "计算算法版本。",
    "amount": "成交金额，单位为人民币元。",
    "amount_cny": "成交金额，单位为人民币元。",
    "batch_code": "采集批次代码，格式为 HHMMSS。",
    "bias_5_pct": "当日收盘价相对五日均线的乖离率，单位为百分比。",
    "bias_direction": "乖离率相对上一交易日的变化方向。",
    "bias_sample_count": "用于近三十日极值计算的有效样本数。",
    "board_code": "板块的六位代码。",
    "board_id": "带市场前缀的板块标识。",
    "board_name": "板块名称。",
    "breakout_pct": "收盘价超过此前一百一十九日最高收盘价的幅度，单位为百分比。",
    "by_rule": "按规则分组的质量问题数量。",
    "cache_hit": "本次结果是否命中服务内缓存。",
    "calculation_id": "计算批次标识。",
    "calculation_mode": "计算模式。",
    "candidate_count": "候选股票数量。",
    "change_pct": "相对上一交易日收盘价的涨跌幅，单位为百分比。",
    "change_percent": "涨跌幅，单位为百分比。",
    "close": "收盘价。",
    "closing_bid1_sealing_amount_cny": "按收盘买一价和买一量计算的封单额，单位为人民币元。",
    "closing_ask1_sealing_amount_cny": "按收盘卖一价和卖一量计算的跌停封单额，单位为人民币元。",
    "code": "不含交易所前缀的六位股票代码。",
    "codes": "不含交易所前缀的六位股票代码集合，最多五百个。",
    "collected_at": "实际完成采集的时间。",
    "comparison_session_count": "用于比较的历史交易日数量。",
    "consecutive_limit_up_days": "连续涨停交易日数量。",
    "consecutive_limit_down_days": "连续跌停交易日数量。",
    "count": "返回记录总数。",
    "cumulative_amount": "截至快照时点的累计金额，单位为人民币元。",
    "cumulative_amount_cny": "截至快照时点的累计成交金额，单位为人民币元。",
    "cumulative_volume": "截至快照时点的累计数量，单位为股。",
    "cumulative_volume_shares": "截至快照时点的累计成交量，单位为股。",
    "current_name": "证券当前名称。",
    "daily_bar_ingestion_id": "日线数据采集批次标识。",
    "data_origin": "本次返回数据的来源。",
    "database_persistence": "数据库异步持久化状态。",
    "delisting_date": "退市日期；未退市时为空。",
    "direction": "涨停或跌停方向。",
    "displayed_volume_shares": "来源页面展示的竞价匹配量，单位为股。",
    "down": "一字跌停股票列表。",
    "down_count": "一字跌停股票数量。",
    "duration_semantics": "封板持续时间字段的统计口径。",
    "effective_trade_date": "结果实际对应的交易日。",
    "eligible_count": "满足计算条件的股票数量。",
    "eligible_history_count": "具备完整历史数据的股票数量。",
    "end_close": "区间结束交易日的收盘价。",
    "end_date": "查询结束日期，包含当日。",
    "end_trade_date": "计算区间的结束交易日。",
    "error": "错误详情。",
    "exchange": "证券所属交易所。",
    "expected_rounds": "竞价序列应包含的采集轮次数。",
    "final_batch_code": "抢筹线计算使用的结束批次，固定为092520。",
    "fetched_at": "数据获取时间，按上海时区格式化。",
    "first_batch_code": "抢筹线计算使用的起始批次，固定为092453。",
    "first_limit_up_at": "当日首次涨停时间。",
    "first_limit_down_at": "当日首次跌停封板时间；来源未提供时为空。",
    "free_float_market_cap_cny": "流通市值，单位为人民币元。",
    "free_float_shares": "自由流通股本，单位为股。",
    "free_float_turnover_rate_pct": "自由流通换手率，单位为百分比。",
    "found_count": "查询到最新每日指标的股票数量。",
    "generated_at": "结果生成时间。",
    "grab_line_pct": "抢筹线百分比，即（09:25:20价格－09:24:53价格）除以昨收价再乘100。",
    "has_more": "是否还有未返回的记录。",
    "high": "最高价。",
    "high_price": "截至快照时点的最高价。",
    "highest_bias_5_pct": "近三十个交易日内的最高五日线乖离率。",
    "highest_bias_trade_date": "最高五日线乖离率对应的交易日。",
    "hot_money_name": "经审核目录中的游资标准名称。",
    "incomplete_history": "因历史数据不足而省略的股票数量。",
    "indicated_price": "竞价指示价格。",
    "indicative_price": "竞价虚拟匹配价格。",
    "indicator_ingestion_id": "指标来源采集批次标识。",
    "ingestion_id": "采集批次标识。",
    "ingestion_status": "所选采集批次的状态。",
    "input_hash": "用于追溯计算输入的哈希值。",
    "ipo_date": "上市日期。",
    "is_exchange_trade_tick": "是否为交易所成交明细；本接口固定为否。",
    "is_order_by_order": "是否为逐笔委托数据；本接口固定为否。",
    "is_st": "当日是否为风险警示证券。",
    "items": "返回的数据记录列表。",
    "last_limit_up_at": "当日最后一次涨停时间。",
    "last_limit_down_at": "当日最后一次跌停封板时间。",
    "last_price": "快照时点价格；竞价阶段按既定口径取值。",
    "level": "盘口档位序号。",
    "limit_price": "按价格限制规则计算的涨停价或跌停价。",
    "limit_up_duration_seconds": "当日涨停持续时间，单位为秒。",
    "limit_down_duration_seconds": "当日跌停累计封板时长；无事件流时为空。",
    "live_provider_derived": "结果是否包含实时来源数据的计算值。",
    "low": "最低价。",
    "low_price": "截至快照时点的最低价。",
    "lowest_bias_5_pct": "近三十个交易日内的最低五日线乖离率。",
    "lowest_bias_trade_date": "最低五日线乖离率对应的交易日。",
    "member_count": "快照中的成员总数。",
    "members": "分类成员证券代码列表。",
    "message": "面向调用方的错误说明。",
    "max_age_seconds": "为兼容既有客户端保留的字段；腾讯请求时实时读取不使用该值筛选结果。",
    "missing_close": "因缺少收盘价而省略的记录数。",
    "missing_codes": "请求中未查询到数据的六位股票代码。",
    "missing_end_bar": "因缺少区间结束日行情而省略的记录数。",
    "missing_free_float_shares": "因缺少自由流通股本而省略的记录数。",
    "missing_name": "因缺少证券名称而省略的记录数。",
    "missing_start_bar": "因缺少区间开始日行情而省略的记录数。",
    "moving_average_5": "最近五个有效交易日收盘价的简单移动平均值。",
    "name": "证券或板块名称。",
    "n": "抢筹线筛选阈值N，单位为百分比，默认0。",
    "name_ingestion_id": "证券名称来源采集批次标识。",
    "non_trading_bar": "因行情不属于有效交易日而省略的记录数。",
    "nonpositive_price": "因价格不大于零而省略的记录数。",
    "observed_at": "行情观察时间，按上海时区格式化。",
    "one_price": "观察窗口内二十九轮完全一致的集合竞价价格。",
    "offset": "从结果集起点跳过的记录数。",
    "omission_reasons": "按原因汇总的省略记录数量。",
    "omissions": "本次计算省略记录的汇总。",
    "omitted_count": "因数据质量或业务规则而省略的记录数。",
    "omitted_incomplete_count": "因证据字段不完整而省略的记录数。",
    "open": "开盘价。",
    "open_count": "涨停打开次数。",
    "order_book_ingestion_id": "盘口数据采集批次标识。",
    "persistence_status": "异步持久化状态。",
    "pool_calculation_id": "股票池计算批次标识。",
    "previous_119d_high": "当日前一百一十九个交易日的最高收盘价。",
    "previous_bias_5_pct": "上一有效交易日的五日线乖离率。",
    "previous_close": "上一交易日收盘价。",
    "price": "盘口档位价格。",
    "previous_trade_date": "上一有效交易日。",
    "price_limit_algorithm_version": "涨跌停价格计算算法版本。",
    "price_limit_calculation_id": "涨跌停价格计算批次标识。",
    "price_limit_rule_version": "涨跌停价格规则版本。",
    "price_limit_status": "数据源发布的当日价格涨跌停状态。",
    "quality": "数据质量摘要。",
    "quote_status": "来源行情状态。",
    "raw_captured": "是否已经保存不可变原始数据。",
    "raw_id": "不可变原始数据对象标识。",
    "rejected_count": "未通过校验的候选记录数。",
    "requested_count": "请求的股票代码数量。",
    "return_interval_count": "收益率计算包含的相邻交易日区间数。",
    "return_pct": "区间收益率，单位为百分比。",
    "returned_count": "本次实际返回的记录数。",
    "returned_rounds": "本次实际返回的采集轮次数。",
    "round_status": "本轮采集状态。",
    "round_count": "参与形态判定的成功采样轮次数，固定为二十九。",
    "rounds": "按计划执行时间正序排列的采集轮次。",
    "rule_version": "业务规则版本。",
    "sample_seq": "采集轮次序号。",
    "sample_count": "该股票参与形态判定的有效采样数，固定为二十九。",
    "scheduled_at": "计划采集时间。",
    "seal_amount": "竞价封单额，按买一价乘以买一量计算，单位为人民币元。",
    "security_type": "证券类型。",
    "selected_ingestion_id": "本次查询选中的采集批次标识。",
    "semantics": "数据语义标识。",
    "service": "服务名称。",
    "session_id": "竞价序列采集会话标识。",
    "session_status": "竞价序列采集会话状态。",
    "snapshot_date": "分类快照日期。",
    "snapshot_id": "不可变快照标识。",
    "snapshot_window": "允许参与计算的快照时间窗口。",
    "source": "数据来源。",
    "source_display_classification": "来源页面展示的分类；不代表成交方向。",
    "source_display_classification_trusted": "来源展示分类是否可作为可信交易方向。",
    "source_ingestion_id": "来源数据采集批次标识。",
    "source_observation_ingestion_id": "来源观察记录的采集批次标识。",
    "source_observation_raw_id": "来源观察记录的原始数据标识。",
    "source_reported_sealed_funds_cny": "数据源报告的封单资金，单位为人民币元。",
    "source_row_count": "数据源返回的原始记录数。",
    "source_sequence": "来源记录的稳定排序序号。",
    "source_timestamp": "腾讯来源标注的行情时间。",
    "start_close": "区间开始交易日的收盘价。",
    "min_grab_line_pct": "抢筹线开区间最小值，单位为百分比。",
    "max_grab_line_pct": "抢筹线开区间最大值；null 表示不限制。",
    "min_change_pct": "09:25:20 涨跌幅开区间最小值，单位为百分比。",
    "max_change_pct": "09:25:20 涨跌幅开区间最大值；null 表示不限制。",
    "change_pct_092520": "09:25:20 价格相对昨收的涨跌幅，单位为百分比。",
    "start_date": "查询开始日期，包含当日。",
    "start_trade_date": "计算区间的开始交易日。",
    "status": "状态。",
    "stock_name": "股票当前名称。",
    "symbol": "带交易所前缀的标准证券代码。",
    "total_candidate_count": "进入计算的候选股票总数。",
    "total_market_value": "总市值，单位为人民币元。",
    "total_shares": "总股本，单位为股。",
    "total_count": "符合查询条件的记录总数。",
    "total_findings": "数据质量问题总数。",
    "trade_date": "交易日。",
    "trade_status": "证券当日交易状态。",
    "trading_session_count": "计算窗口包含的交易日数量。",
    "up": "一字涨停股票列表。",
    "up_count": "一字涨停股票数量。",
    "valid_count": "通过校验的记录数。",
    "value_semantics": "竞价快照价格和数量字段采用的语义口径。",
    "version": "不可变结果版本。",
    "volume": "成交量，单位为股。",
    "volume_shares": "盘口档位数量，单位为股。",
    "volume_ratio": "量比，无量纲。",
    "window_end": "形态观察窗口结束时间。",
    "window_start": "形态观察窗口开始时间。",
    "window_trading_days": "近端统计窗口的交易日数量。",
    "window_trading_session_count": "新高判断窗口包含的交易日数量。",
    # Query/path parameters that are not response-model fields.
    "as_of_date": "查询采用的有效日期。",
    "classification_code": "分类代码。",
    "classification_type": "分类类型。",
    "limit": "最多返回的记录数。",
    "namespace": "分类体系命名空间。",
    "query": "证券代码或名称搜索词。",
    "turnover_rate_pct": "换手率，单位为百分比。",
    "circulating_shares": "流通股本，单位为股。",
    "circulating_market_value": "流通市值，单位为人民币元。",
    "pe": "市盈率，倍数。",
    "pe_ttm": "滚动市盈率，倍数。",
    "pb": "市净率，倍数。",
    "ps": "市销率，倍数。",
    "ps_ttm": "滚动市销率，倍数。",
    "dividend_yield_pct": "股息率，单位为百分比。",
    "dividend_yield_ttm_pct": "滚动股息率，单位为百分比。",
    "bid_levels": "买一至买五盘口，按档位升序排列。",
    "ask_levels": "卖一至卖五盘口，按档位升序排列。",
    "buy_amount": "买入金额，单位为人民币元。",
    "buy_rank": "席位在买入榜中的名次；未进入买入榜时为空。",
    "buy_seat_count": "进入买入榜的席位数量。",
    "buy_sell_overlap_count": "同时进入买入榜和卖出榜的稳定席位数量。",
    "buy_seats": "买入金额前五席位明细。",
    "buy_to_market_pct": "席位买入金额占当日市场成交额的比例，单位为百分比。",
    "change_rate_pct": "涨跌幅，单位为百分比。",
    "close_price": "当日收盘价。",
    "amplitude": "统计周期内振幅，单位为百分比。",
    "event_id": "标准化龙虎榜事件 UUID。",
    "is_institution": "来源是否明确披露为机构席位。",
    "is_northbound": "来源是否明确披露为沪股通或深股通专用席位。",
    "institution_buy_amount": "机构席位可完整计算的买入合计，单位为人民币元；披露不完整时为空。",
    "institution_sell_amount": "机构席位可完整计算的卖出合计，单位为人民币元；披露不完整时为空。",
    "institution_net_amount": "机构席位可完整计算的买卖净额，单位为人民币元；披露不完整时为空。",
    "lhb_buy_amount": "龙虎榜披露的买入合计，单位为人民币元。",
    "lhb_sell_amount": "龙虎榜披露的卖出合计，单位为人民币元。",
    "net_buy_strength": "可计算净买额除以证券成交额；分母或金额缺失时为空。",
    "northbound_buy_amount": "北向专用席位可完整计算的买入合计，单位为人民币元；披露不完整时为空。",
    "northbound_sell_amount": (
        "北向专用席位可完整计算的卖出合计，单位为人民币元；披露不完整时为空。"
    ),
    "northbound_net_amount": "北向专用席位可完整计算的买卖净额，单位为人民币元；披露不完整时为空。",
    "trigger_window_basis": (
        "触发条件窗口口径；MARKET_SESSIONS 为市场交易日，"
        "SECURITY_TRADED_SESSIONS 为证券实际交易日。"
    ),
    "trigger_window_sessions": "触发条件窗口包含的交易会话数。",
    "trigger_occurrence_count": "来源披露的触发条件发生次数；未披露时为空。",
    "trigger_start_date": "触发条件窗口开始交易日；来源无法可靠确定时为空。",
    "trigger_end_date": "触发条件窗口结束交易日。",
    "amount_period_basis": "金额统计区间口径；无法可靠确定时为 SOURCE_UNSPECIFIED。",
    "amount_period_sessions": "金额统计区间包含的交易会话数；未披露时为空。",
    "amount_period_start_date": "金额统计区间开始交易日；未披露时为空。",
    "amount_period_end_date": "金额统计区间结束交易日；未披露时为空。",
    "amount_period_verified": "金额统计区间是否已由来源信息可靠确认。",
    "buy_disclosure_present": "来源是否提供了可用的买入方向披露。",
    "sell_disclosure_present": "来源是否提供了可用的卖出方向披露。",
    "data_quality_codes": "与该龙虎榜事实关联的数据质量代码列表。",
    "pure_buy_seat_count": "买入额大于零且卖出额明确为零的席位数量。",
    "pure_sell_seat_count": "卖出额大于零且买入额明确为零的席位数量。",
    "deal_amount": "龙虎榜成交金额，单位为人民币元。",
    "deal_to_market_pct": "龙虎榜成交金额占当日市场成交额的比例，单位为百分比。",
    "free_float_market_value": "自由流通市值，单位为人民币元。",
    "market_amount": "证券当日市场成交额，单位为人民币元。",
    "net_amount": "净买入金额，单位为人民币元。",
    "net_to_market_pct": "净买入金额占当日市场成交额的比例，单位为百分比。",
    "rank": "席位在买入或卖出方向中的名次。",
    "reason_code": "上榜原因代码。",
    "reason_name": "标准化上榜原因名称。",
    "reason_name_raw": "数据源披露的原始上榜原因名称。",
    "reason_type": "标准化上榜原因类别。",
    "reason_text": "上榜原因说明。",
    "seat_code": "数据源提供的营业部席位代码。",
    "seat_id": "标准化后的稳定席位 UUID；无法可靠识别时为空。",
    "seat_name": "数据源提供的营业部席位名称。",
    "seat_name_raw": "数据源披露的原始席位名称。",
    "seat_trade_id": "标准化席位行为 UUID。",
    "seat_trades": "事件内合并后的席位买卖行为列表。",
    "sell_amount": "卖出金额，单位为人民币元。",
    "sell_rank": "席位在卖出榜中的名次；未进入卖出榜时为空。",
    "sell_seat_count": "进入卖出榜的席位数量。",
    "sell_seats": "卖出金额前五席位明细。",
    "sell_to_market_pct": "席位卖出金额占当日市场成交额的比例，单位为百分比。",
    "side": "席位方向，buy 表示买入，sell 表示卖出。",
    "source_code": "数据来源代码；龙虎榜事实可来自 eastmoney 或 tushare。",
    "source_record_id": "数据源记录的稳定追踪标识。",
    "source_event_id": "数据源龙虎榜事件标识。",
    "summary_buy_amount": "所属证券上榜汇总的买入金额，单位为人民币元。",
    "summary_deal_amount": "所属证券上榜汇总的成交金额，单位为人民币元。",
    "summary_net_amount": "所属证券上榜汇总的净买入金额，单位为人民币元。",
    "summary_sell_amount": "所属证券上榜汇总的卖出金额，单位为人民币元。",
    "top1_buy_concentration": "买入榜第一名金额占龙虎榜买入合计的比例。",
    "top3_buy_concentration": "买入榜前三名金额占龙虎榜买入合计的比例。",
    "top5_buy_concentration": "买入榜前五名金额占龙虎榜买入合计的比例。",
    "top1_sell_concentration": "卖出榜第一名金额占龙虎榜卖出合计的比例。",
    "top3_sell_concentration": "卖出榜前三名金额占龙虎榜卖出合计的比例。",
    "top5_sell_concentration": "卖出榜前五名金额占龙虎榜卖出合计的比例。",
    "turnover_amount": "证券在统计周期内的成交金额，单位为人民币元。",
    "turnover_rate": "证券在统计周期内的换手率，单位为百分比。",
}


for side_en, side_zh in (("bid", "买"), ("ask", "卖")):
    for level, level_zh in enumerate(("一", "二", "三", "四", "五"), start=1):
        FIELD_DESCRIPTIONS_ZH[f"{side_en}{level}_price"] = f"{side_zh}{level_zh}价。"
        FIELD_DESCRIPTIONS_ZH[f"{side_en}{level}_volume"] = (
            f"{side_zh}{level_zh}量，单位为股；价格为空或为零时仍保留来源返回的数量。"
        )

for level, level_zh in enumerate(("一", "二", "三", "四", "五"), start=1):
    FIELD_DESCRIPTIONS_ZH[f"closing_bid{level}_price"] = f"收盘买{level_zh}价。"
    FIELD_DESCRIPTIONS_ZH[f"closing_bid{level}_volume_shares"] = f"收盘买{level_zh}量，单位为股。"
    FIELD_DESCRIPTIONS_ZH[f"closing_ask{level}_price"] = f"收盘卖{level_zh}价。"
    FIELD_DESCRIPTIONS_ZH[f"closing_ask{level}_volume_shares"] = f"收盘卖{level_zh}量，单位为股。"


def localize_openapi(schema: dict[str, Any]) -> dict[str, Any]:
    """Attach Chinese descriptions to owned fields and parameters, failing closed."""

    for path_item in schema["paths"].values():
        for method, operation in path_item.items():
            if method == "parameters":
                continue
            for parameter in operation.get("parameters", []):
                name = parameter["name"]
                parameter["description"] = _description(name)

    for name, component in schema["components"]["schemas"].items():
        if name in {"HTTPValidationError", "ValidationError"}:
            continue
        for field_name, field_schema in component.get("properties", {}).items():
            if not _contains_chinese(field_schema.get("description")):
                field_schema["description"] = _description(field_name)

    return schema


def _description(field_name: str) -> str:
    try:
        return FIELD_DESCRIPTIONS_ZH[field_name]
    except KeyError as error:
        raise RuntimeError(f"missing Chinese OpenAPI description for {field_name!r}") from error


def _contains_chinese(value: object) -> bool:
    return isinstance(value, str) and any("\u4e00" <= char <= "\u9fff" for char in value)
