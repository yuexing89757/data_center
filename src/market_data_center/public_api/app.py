...
def create_app(
    *,
    ...
    @app.post(
        ...
        description=(
            "返回指定交易日 09:25:20 序列末批的数据；优先使用成功 attempt，仅在没有成功"
            "attempt 时使用所选 partial attempt。同一个六位代码可能同时匹配沪市和深市证券，"
            "未命中的代码会明确列出。不使用其他日期、较早轮次或历史单次快照回退。"
        ),
    )
    ...
    @app.get(
        ...
        tags=["市场数据"],
        summary="实时计算 09:25:20 主板一字涨跌停列表",
        description=(
            "选取已存储的沪深市场 09:25:20 竞价序列末批，在读取时按已接受的主板百分之十"
            "涨跌停规则计算结果。不依赖历史单次快照或夜间涨跌停批次，不访问行情提供方，"
            "不使用更晚的日线，也不写入数据。"
        ),
    )
