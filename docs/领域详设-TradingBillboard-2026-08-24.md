...
2. 汇总 `symbol` 不在当日已知股票集合时，其汇总和席位只保留 Raw、计入过滤数量，不产生
   QualityResult，也不阻断其他已知证券。
3. 北交所证券、可转债和其他未知证券不产生标准记录。过滤后至少必须保留一条标准记录；整批全部
   被过滤时以 `trading_billboard.no_known_security` 失败，禁止空成功。
...
未知证券过滤后，任一已知证券硬规则失败时，记录 QualityResult 并使整个日期失败。不得写半套
汇总、孤立席位或空成功，不得切换 Provider。
...
- 混入北交所、可转债和未知股票时跳过未知证券但保留已知证券，以及生命周期和非交易日硬失败；
