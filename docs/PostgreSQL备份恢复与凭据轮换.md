...
完整的 PostgreSQL 灾备还应按 roles、schema 和 data 分别导出。使用 `pg_dump` / PostgreSQL 工具按仓库迁移方式分别导出，避免在恢复时引入不必要的内部对象或权限错误：

```bash
pg_dump "$SOURCE_DATABASE_URL" -f roles.sql --role-only
pg_dump "$SOURCE_DATABASE_URL" -f schema.sql --schema-only
pg_dump "$SOURCE_DATABASE_URL" -f data.sql --column-inserts --data-only
```

连接串会成为进程参数，因此这些完整导出命令只能在受控运维主机执行；执行期间应限制进程查看权限，结束后立即清理环境变量和 shell 历史中的展开值。

参考：[PostgreSQL 文档 - Backup and Restore](https://www.postgresql.org/docs/current/backup.html)。
...
所有轮换在维护窗口执行。轮换前确认可登录数据库主机、已有最近一次通过恢复验证的备份、旧值仍可从密钥管理器回滚，并列出所有消费者。轮换后更新密钥管理器和服务环境，重建容器，检查健康状态、数据库连接、PostgREST 只读查询及采集写入。

### 数据库密码

通过 PostgreSQL 的 `ALTER ROLE` 或部署环境的密码管理流程修改，然后重建依赖服务使其重新建立连接：

```bash
psql "$DATABASE_URL" -c "ALTER ROLE market_data_worker WITH PASSWORD '<new-password>';"
```

若验证失败，在密钥管理器恢复旧密码和服务配置，再次重建。参考：[PostgreSQL 文档 - ALTER ROLE](https://www.postgresql.org/docs/current/sql-alterrole.html)。

### API key 与 JWT 签名密钥

FastAPI 的 `X-API-Key` 由部署环境管理，更新密钥管理器和服务环境后重建服务即可。先部署同时接受新旧密钥的过渡配置，再逐个更新消费者，最后撤销旧密钥。

> 注：本项目不再使用 Supabase Auth / Studio，相关密钥轮换不再适用。

## 5. 数据库网络收口

生产 PostgreSQL 不应对公网任意来源开放。首选取消数据库端口的公网映射，让应用通过内部网络或反向代理访问；确需直接运维连接时，只允许固定管理出口 IP 或 VPN 网段。

执行顺序：

1. 记录当前防火墙和容器端口配置，但不记录凭据；
2. 从允许网段验证数据库、PostgREST 和采集任务；
