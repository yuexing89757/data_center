...
后续决定：2026-09-22 的 ADR-0057 替代下述单条非法标准记录阻断全部合法事实的行为。
日期、人数、来源及唯一性不变量不变；非法行隔离，合法行可发布，任务保留 partial/failed。
...
股东人数标准事实范围收窄为上交所和深交所股票。Tushare 全市场响应中的北交所行完整保留在
Raw，但登记 `shareholder_count.unsupported_exchange` 后不进入 Core，也不使当日批次硬失败；
历史回填目标同样只包含 `SSE` 和 `SZSE`。这不改变 Security 领域对北交所证券的统一标识支持。
...
7. 历史回填是显式 CLI Workflow，不注册 Scheduler Job。范围包含沪、深全部已知股票，
