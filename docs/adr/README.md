...
- `ADR-0054-竞价序列快照六个月历史归档.md`：02:30幂等归档32轮竞价序列标准事实，03:00清理前逐键校验，历史表滚动保留六个自然月且不新增公共API。
- `ADR-0053-DragonTiger历史覆盖与质量修复.md`：以通用触发窗口和独立金额周期修复龙虎榜历史覆盖，按来源代码稳定席位身份，保留并审计重复/占位/单侧披露，引入分阶段 Raw 血缘、BSE 证券任务和替换读取契约。
- `ADR-0052-竞价末批复用与单次快照任务退役.md`：两个公共竞价接口固定读取 09:25:20 序列末批并按盘口计算封单额；退役 09:25:30 重复采集 job，保留历史表和非调度能力。
- `ADR-0051-竞价序列采集与单Writer解耦.md`：以同进程有界 FIFO 和单 PostgreSQL Writer 将 20 秒来源采集时钟与数据库延迟隔离；Writer 失败不停止后续 Raw 采集，历史 replay 仍保持关闭。
- `ADR-0050-竞价序列快照三交易日保留与清理.md`：Worker 每天 03:00 仅清理沪深全市场竞价序列明细，固定保留最近三个已完成交易日；会话、轮次、Raw、lineage、质量与历史单次快照继续保留。
- `ADR-0049-DragonTiger事实与时点安全特征.md`：以统一事件、原因、稳定席位及席位行为事实替换 TradingBillboard v1；新增普通榜/三日榜周期、客观资金指标和时点安全特征，并删除旧三个读取契约。
- `ADR-0048-沪深主板与创业板监管异动规则测算.md`：新增 Regulation 领域，以2026年7月6日起生效的沪深交易所现行规则，对沪深主板和创业板分别保存官方事件、版本化规则结果与三个指数情景下的T+1客观触发条件；不预测价格或监管措施。
...
- `ADR-0046-东方财富股票龙虎榜采集与只读契约.md`：已由 ADR-0049 替代；仅作为 TradingBillboard v1 历史决策保留。
...
- `ADR-0028-暂停全市场竞价Raw重放与移除自动最终化.md`：其 Raw replay 和 21:30 自动最终化决策继续有效；09:25:30 五档来源采集已由 ADR-0052 退役。
...
- `ADR-0034-沪深全市场开盘竞价序列快照.md`：接受工作日全市场竞价序列采集；2026-09-07 起最后四轮为 09:24:20、09:24:40、09:24:53、09:25:20，末轮公共读取与单次快照任务退役由 ADR-0052 补充。
- `ADR-0039-09点26沪深主板一字涨跌停实时计算.md`：取代 ADR-0032 的晚间 CalculationRun 依赖；其来源已由 ADR-0052 改为 09:25:20 序列末批。
