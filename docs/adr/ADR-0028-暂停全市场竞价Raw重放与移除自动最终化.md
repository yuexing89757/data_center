...
1. 晨间 `call-auction-market-snapshot-daily` 继续在工作日 09:25:50 采集，沿用
...
5. 不新增替代时间、`.env` hour/minute、cron、systemd timer、Windows Task Scheduler 或其他自动执行。
   `CALL_AUCTION_SNAPSHOT_ENABLED` 只控制 09:25:50 晨间来源采集。
...
- Worker 和本地任务页只显示 09:25:50 全市场来源采集，不显示 21:30 自动最终化；
...
- 09:25:50 job 仍受现有布尔开关控制，`.env` 无时间字段；
