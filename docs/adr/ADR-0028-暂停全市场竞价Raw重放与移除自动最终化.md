...
> 2026-08-18 澄清：项目所有者将晨间单次快照固定触发时点调整为工作日
> `09:25:50 Asia/Shanghai`。快照保存 PYTDX 买卖五档；档位价格为零但数量为正时价格保存为
> `NULL` 且数量保留。卖一量为空或零时，以买一价乘买一股数计算 `seal_amount`，缺少买一价量
> 时结果为 `NULL`。`.env` 仍只保留启停开关。

> 2026-08-19 澄清（Issue #59）：晨间单次快照从下一交易日 `2026-08-20` 起固定为工作日
> `09:25:30 Asia/Shanghai`。仅当卖一、卖二、卖三量分别都为 `NULL` 或零时，才视为涨停封单状态；
> 买一价量完整时 `seal_amount = bid1_price * bid1_volume`，否则为 `NULL`。2026-08-19 及以前的
> 不可变来源事实继续按原卖一量规则校验，不回写历史。`.env` 仍只控制启停。
...
1. 晨间 `call-auction-market-snapshot-daily` 从 2026-08-20 起在工作日 09:25:30 采集，沿用
...
5. 不新增替代时间、`.env` hour/minute、cron、systemd timer、Windows Task Scheduler 或其他自动执行。
   `CALL_AUCTION_SNAPSHOT_ENABLED` 只控制 09:25:30 晨间来源采集。
...
- Worker 和本地任务页只显示 09:25:30 全市场来源采集，不显示 21:30 自动最终化；
...
- 09:25:30 job 仍受现有布尔开关控制，`.env` 无时间字段；
