...
ADR-0048 已接受 `Regulation` 监管异动规则测算领域。首版从2026年7月6日起，对沪市主板、
深市主板和创业板普通股票分别保存交易所官方异常事件、版本化逐规则计算结果及固定指数情景下
的T+1客观触发条件；它不预测股票价格，不把规则触发解释为必然监管措施。当前尚未实现，
生产迁移、真实来源采集和Worker调度启用仍需分别授权。
