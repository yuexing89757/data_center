...
- 自然键及内容哈希属于该批次实际输入的 `input_event_keys` 清单；
- 事件来源和证据满足官方事件约束；
- 事件自然键在该水位下唯一。

因此接口不会把 CalculationRun 输入读取后才持久化的事件拼入旧计算版本，包括保留旧观察时间的
Raw 重放。清单由 Worker 从同一份计算输入提取并与新批次原子保存，不在查询时猜测。
`[]` 是已确认无事件，`null` 是旧批次未捕获；后者返回 404，不回退、不自动补造或重算历史。
晚到事件必须参与新的计算版本，才能出现在接口响应中。
