...
from market_data_center.scheduler import (
    ...
    _require_dragon_tiger_profile_prerequisites,
    build_scheduler,
...
def test_scheduler_registers_persistent_single_instance_market_job(tmp_path: Path) -> None:
    ...
    archive = scheduler.get_job(CALL_AUCTION_MARKET_SERIES_ARCHIVE_JOB_ID)
    assert archive is not None
    assert str(archive.trigger) == "cron[hour='2', minute='30']"
    assert archive.executor == "default"
    assert archive.coalesce
    assert archive.max_instances == 1
...
def test_retired_call_auction_market_snapshot_job_is_not_registered(tmp_path: Path) -> None:
    scheduler = build_scheduler(
        SchedulerSettings(scheduler_store_path=tmp_path / "retired-auction.sqlite", _env_file=None)
    )

    assert scheduler.get_job("call-auction-market-snapshot-daily") is None
    assert not hasattr(scheduler_module, "run_call_auction_market_snapshot_job")
...
def test_dragon_tiger_seat_profile_schedule_is_opt_in_and_fixed_at_2100(tmp_path: Path) -> None:
    disabled = build_scheduler(
        SchedulerSettings(scheduler_store_path=tmp_path / "profile-disabled.sqlite", _env_file=None)
    )
    enabled = build_scheduler(
        SchedulerSettings(
            scheduler_store_path=tmp_path / "profile-enabled.sqlite",
            dragon_tiger_seat_profile_enabled=True,
            _env_file=None,
        )
    )

    assert disabled.get_job(DRAGON_TIGER_SEAT_PROFILE_JOB_ID) is None
    job = enabled.get_job(DRAGON_TIGER_SEAT_PROFILE_JOB_ID)
    assert job is not None
    assert str(job.trigger) == "cron[day_of_week='mon-fri', hour='21', minute='0']"
    assert job.max_instances == 1
...
def test_scheduled_job_fire_time_uses_catalog_definition(monkeypatch) -> None:
    ...
    assert captured == [(9, 15, 0, "Asia/Shanghai", True)]
...
def test_disabling_series_leaves_no_morning_auction_collection_job(tmp_path: Path) -> None:
    scheduler = build_scheduler(
        SchedulerSettings(
...
def test_auction_series_archive_job_can_only_be_enabled_or_disabled(tmp_path: Path) -> None:
    scheduler = build_scheduler(
        SchedulerSettings(
            scheduler_store_path=tmp_path / "no-series-archive.sqlite",
            call_auction_market_series_archive_enabled=False,
            _env_file=None,
        )
    )

    assert scheduler.get_job(CALL_AUCTION_MARKET_SERIES_ARCHIVE_JOB_ID) is None
    assert scheduler.get_job(DATA_CLEANUP_JOB_ID) is not None
...
def test_auction_series_archive_runner_uses_shanghai_reference_date_and_records_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fire_time = datetime(2026, 9, 2, 18, 30, tzinfo=UTC)
    engine = SimpleNamespace(disposed=False)
    engine.dispose = lambda: setattr(engine, "disposed", True)
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(scheduler_module, "WorkflowExecutionService", FakeExecutionService)
    monkeypatch.setattr(scheduler_module, "AuctionSeriesArchiveService", FakeService)
    monkeypatch.setattr(
        scheduler_module,
        "_scheduled_job_fire_time",
        lambda *args, **kwargs: fire_time,
    )

    scheduler_module.run_call_auction_market_series_archive_job()

    assert captured["workflow"] == (
        scheduler_module.WorkflowCode.CALL_AUCTION_MARKET_SERIES_ARCHIVE,
        fire_time,
        scheduler_module.TriggerSource.SCHEDULED,
    )
    assert captured["persistence"] == ("facts", engine)
    assert captured["step"] == ("archive_call_auction_market_series_snapshots", 1)
    assert captured["reference_date"] == date(2026, 9, 3)
    assert captured["succeeded"] is True
    assert engine.disposed is True
...
def test_scheduler_removes_persisted_retired_jobs(tmp_path: Path) -> None:
    ...
    build_scheduler(SchedulerSettings(scheduler_store_path=store_path, _env_file=None))
