-- Return every member of the latest ready daily limit-up snapshot.
-- The original migration was already applied with LIMIT 1 on the member query,
-- so this correction is an ordered forward migration.
...
as $$
    ...
    enriched as (
        select
            ...
            bar.close,
            bar.volume,
            case when bar.close is not null and ind.free_float_shares is not null
                then bar.close * ind.free_float_shares
            end as free_float_market_cap,
            ind.free_float_turnover_rate_pct,
            eod.seal_amount,
            case when eod.bid1_volume is not null and bar.volume is not null and bar.volume > 0
        ...
        left join core.stock_daily_indicator ind
          on ind.symbol = m.symbol and ind.trade_date = p_trade_date
        left join realtime.eod_quote_snapshot eod
          on eod.symbol = m.symbol and eod.trade_date = p_trade_date
        left join realtime.call_auction_snapshot ca
          on ca.symbol = m.symbol and ca.trade_date = p_trade_date
        order by m.symbol
...
begin
    if exists (select 1 from pg_roles where rolname = 'market_data_api') then
        grant execute on function api_v1.query_daily_limit_up_list(date, integer)
            to market_data_api;
    end if;
