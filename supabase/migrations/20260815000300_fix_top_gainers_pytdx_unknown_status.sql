create or replace function api_v1.query_top_gainers_20d(
...
begin
    ...
    with candidates as (
        ...
        from facts where start_close > 0 and end_close > 0
          and start_status in ('trading', 'unknown')
          and end_status in ('trading', 'unknown')
        ...
        select count(*)::integer total_candidate_count,
          ...
          count(*) filter(where (start_status is not null
                                  and start_status not in ('trading', 'unknown'))
                            or (end_status is not null
                                and end_status not in ('trading', 'unknown')))::integer
              non_trading_bar,
          count(*) filter(where (start_close is not null and start_close<=0)
                            or (end_close is not null and end_close<=0))::integer
              nonpositive_price,
