drop function api_v1.query_call_auction_market_series_snapshots(date, text[]);
...
begin
    if p_trade_date is null
       ...
       or (p_batch_code is not null and p_batch_code !~ '^[0-9]{6}$') then
    ...
    with requested_codes as (
        ...
        where round.session_id = selected_session_id
          and (
              p_batch_code is null
              or to_char(
                  round.scheduled_at at time zone 'Asia/Shanghai', 'HH24MISS'
              ) = p_batch_code
          )
    ), round_payloads as (
...
revoke all on function api_v1.query_call_auction_market_series_snapshots(date, text[], text)
    from public, anon, authenticated;
grant execute on function api_v1.query_call_auction_market_series_snapshots(date, text[], text)
    to market_data_api;
